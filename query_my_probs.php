...
// get the string of uids to query. This is inefficient, but I'm using query_uids, so I'm being lazy.
$get_uids = mysql_query("SELECT uid FROM problems WHERE user='$username' ORDER BY uid");
$uid_array=array();
while($row = mysql_fetch_array($get_uids)) {
	$uid_array[]=$row['uid'];
}
$uid_string=implode(", ",$uid_array);
if(sizeof($uid_array)==0) { 
	echo "<center>The problems you add to CalcDB will appear here.</center>";
	die;
}
...
echo "<br><h3>Problems that you have added to the database:</h3>";
